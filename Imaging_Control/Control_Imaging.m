...
imagem_aux = imagem(:,i); 
figure
imagesc(reshape(imagem_aux', n_pixels , n_pixels)); 
axis equal; 
colormap jet;
...
function ampPhase = multiFreqDemod(signal, Epiv) % signal, frq, sampleLen, sampleFreq or signal, Etot
    phi_tot = Epiv*signal;
    
    phi_tot = reshape(phi_tot, [], 3); % Rearrange to useful order
    ampPhase = zeros(size(phi_tot,1),size(phi_tot,2),size(phi_tot,3));
